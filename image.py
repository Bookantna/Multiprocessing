...
This Image class is referenced from Kylie Ying's Image class in her Image Filters teaching video.
...
class Image:
    def __init__(self, x_pixels=0, y_pixels=0, num_channels=0, path=''):
        # Default input and output will display on main's folder

        # Check if you user created input and output dir already
        direct = os.listdir()
        if 'input' not in direct:
            os.mkdir('input/')
        elif 'output' not in direct:
            os.mkdir('output/')

        self.input_path = 'input/'
        self.output_path = 'output/'
        
        # If user input has x, y and num_channels will create blank_image
        ...
        # If user input path then read that image from that path
        elif path:
            self.array = self.read_image(path)
